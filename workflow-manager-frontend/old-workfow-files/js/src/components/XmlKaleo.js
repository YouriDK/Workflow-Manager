...
export function htmlDecode(input) {
  // Le < ne s'afficher pas correctement en XML il faut cette fonction pour bien convertir
...
export function XML(nodesList, TransitionList, WFid, nom, libelle, envoie) {
  ...
  nodesList.map((node) => {
    ...
    if (node != null) {
      ...
      if (node.name === "Approved") {
        ...
        xw.writeCData(
          '{"xy" :[' + node.x.toPrecision() + "," + node.y.toPrecision() + "]}"
        );
      }
      ...
      if (
        node.extras.actions.length > 0 ||
        node.extras.notifications.length > 0
      ) {
        ...
        if (node.extras.notifications.length > 0) {
          node.extras.notifications.map((element) => {
            ...
            if (element.Notification_Type.length > 0) {
              ...
              xw.writeElement("notification-type", "user-notification");
            }
      ...
      if (node.extras.type === "task") {
        //Plusieurs type de taches donc il fallait la boucle à part
        switch (node.extras.choix) {
          ...
          case "B":
            ...
            xw.endElement("roles"), xw.endElement("assignments");
            break;
          case "C":
            xw.startElement("assignments"),
              xw.startElement("roles"),
          ...
          case "D":
            ...
            break;
          case "E":
            xw.startElement("assignments"), xw.startElement("user");
            ...
            xw.endElement("user"), xw.endElement("assignments");
            break;
          case "F":
            xw.startElement("assignments"),
        ...
        if (node.extras.TaskTimer.length > 0) {
          xw.startElement("task-timers"),
            node.extras.TaskTimer.map((element) => {
              ...
              if (element.block) {
                xw.writeElement("blocking", "true");
              } else {
                xw.writeElement("blocking", "false");
              }
  ...
  xw.endElement("workflow-definition"), xw.endDocument();
