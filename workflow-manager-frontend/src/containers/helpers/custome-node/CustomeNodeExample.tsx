...
const CustomeNodeExample: React.FC<any> = (): JSX.Element => {
  ...
  var link1 = port1.link(node2.getPort(PortModelAlignment.LEFT) as any);
  var link2 = port3.link(node2.getPort(PortModelAlignment.RIGHT) as any);

  var node4 = new DefaultNodeModel('Node 4', 'rgb(0,192,255)');
  var port4 = node4.addOutPort('Out');
  node4.setPosition(200, 10);

  var link3 = port4.link(node2.getPort(PortModelAlignment.TOP) as any);

  var node5 = new DefaultNodeModel('Node 5', 'mediumpurple');
  var port5 = node5.addInPort('In');
  node5.setPosition(400, 300);

  var link4 = port5.link(node2.getPort(PortModelAlignment.BOTTOM) as any);
...
export default CustomeNodeExample;
